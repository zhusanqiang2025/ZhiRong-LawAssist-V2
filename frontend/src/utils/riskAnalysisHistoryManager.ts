...
class RiskAnalysisHistoryManager {
  ...
  async syncHistoryList(): Promise<RiskAnalysisHistoryItem[]> {
    try {
      ...
      // 从后端获取
      const response = await api.get('/api/v1/risk-analysis-v2/sessions');
  ...
  async loadSession(sessionId: string): Promise<RiskAnalysisHistory | null> {
    try {
      const response = await api.get(`/api/v1/risk-analysis-v2/result/${sessionId}`);
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      const response = await api.delete(`/api/v1/risk-analysis-v2/sessions/${sessionId}`);
  ...
  async saveCurrentSession(
    ...
    try {
      await api.patch(`/api/v1/risk-analysis-v2/sessions/${sessionId}/status`, {
        status,
        title
      });
