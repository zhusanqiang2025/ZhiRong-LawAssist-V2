...
const RiskAnalysisPageV2: React.FC = () => {
  ...
  const fetchFinalResult = async (retryCount = 0, specificSessionId?: string) => {
    ...
    try {
      const response = await api.get(`/risk-analysis/result/${sid}`);
  ...
  useEffect(() => {
    const restoreSession = async () => {
      ...
      try {
        ...
        const response = await api.get(`/risk-analysis/${urlSessionId}/restore-state`);
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = async () => {
      try { await api.post(`/risk-analysis/${analysisState.sessionId}/heartbeat`, { ws_connected: true }); }
  ...
  useEffect(() => {
    ...
    const wsUrl = `${getWsBaseUrl()}/api/v1/risk-analysis/ws/${analysisState.sessionId}`;
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      await api.post(`/risk-analysis/start/${session_id}`, { stop_after_preorganization: true });
  ...
  const handleSelectAnalysisMode = async (mode: 'single' | 'multi') => {
    try {
      ...
      await api.post(`/risk-analysis/continue/${analysisState.sessionId}`, {
        analysis_mode: mode,
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const res = await api.post('/risk-analysis/upload', fd);
  ...
  const handleDownloadPreorganizationReport = async () => {
    try {
      const response = await api.get(`/risk-analysis/preorganization-report/${analysisState.sessionId}`, { responseType: 'blob' });
  ...
  const handleDownloadRiskAnalysisReport = async () => {
    try {
      const response = await api.get(`/risk-analysis/report/${analysisState.sessionId}`, { responseType: 'blob' });
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
      const url = window.URL.createObjectURL(blob);
