...
class LegalKnowledgeBase:
    ...
    def _load_common_laws(self):
        ...
        logger.info(f"[法律知识库] 已加载 {sum(len(v) for v in self.articles.values())} 条法律条文")

    def search(self, query: str, legal_domain: str = "") -> List[LegalArticle]:
        """
        在知识库中搜索相关条文

        Args:
            query: 搜索查询
            legal_domain: 法律领域（可选）
        ...
        if legal_domain:
            for domain, articles in self.articles.items():
                if domain in legal_domain or legal_domain in domain:
    ...
    def get_default_articles(self, legal_domain: str) -> List[LegalArticle]:
        """
        获取指定领域的默认条文（当搜索无结果时使用）
...
if __name__ == "__main__":
    ...
    print("测试1：搜索违约相关条文")
    print("=" * 80)
    articles = kb.search("合同违约怎么办", "合同法")
    print(f"找到 {len(articles)} 条相关条文")
    print(kb.format_for_llm(articles))

    print()
    print("=" * 80)
    print("测试2：搜索劳动争议相关条文")
    print("=" * 80)
    articles = kb.search("公司不给我发工资", "劳动法")
    print(f"找到 {len(articles)} 条相关条文")
    print(kb.format_for_llm(articles))
    ...
    print("测试3：获取默认条文")
    print("=" * 80)
    articles = kb.get_default_articles("破产法")
    print(f"找到 {len(articles)} 条默认条文")
    print(kb.format_for_llm(articles))
