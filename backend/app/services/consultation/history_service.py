...
class ConsultationHistoryService:
    ...
    async def get_conversation_with_session_state(
        self,
        user_id: int,
        session_id: str
    ) -> Optional[Dict[str, Any]]:
        """
        获取会话详情（包含 session_state）
        ...
        try:
            ...
            # 确保 session_state 被包含（to_dict 应该已经包含）
            result["session_state"] = history.session_state

            logger.info(f"[历史服务] 获取会话详情(含状态): {session_id}")
