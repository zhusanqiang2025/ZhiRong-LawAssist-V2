...
ContractLegalFeatures = ckg_module.ContractLegalFeatures
ContractTypeDefinition = ckg_module.ContractTypeDefinition
TransactionNature = ckg_module.TransactionNature
ContractObject = ckg_module.ContractObject
Complexity = ckg_module.Complexity
Stance = ckg_module.Stance
ConsiderationType = ckg_module.ConsiderationType
...
TRANSACTION_NATURE_MAP = {
    ...
    "借款融资": TransactionNature.CAPITAL_FINANCE,
    "合伙/联营": TransactionNature.ENTITY_CREATION,
    "公司治理": TransactionNature.SERVICE_DELIVERY,
}
...
CONTRACT_OBJECT_MAP = {
    ...
    "财产": ContractObject.MONETARY_DEBT,  # 财产相关映射到资金
}
...
def update_knowledge_graph_from_json(json_file: str):
    ...
    for ct_data in contract_types_list:
        try:
            ...
            # 创建 ContractTypeDefinition 对象
            contract_type_def = ContractTypeDefinition(
                name=name,
                aliases=aliases,
                category=category,
            ...
            # 添加到知识图谱
            kg._contract_types[name] = contract_type_def
            added += 1
            print(f"  [OK] 添加: {name} ({category})")

        except Exception as e:
            errors.append(f"{name}: {str(e)}")
            print(f"  [ERROR] 错误: {name} - {str(e)}")
            import traceback
            traceback.print_exc()

    # 保存到文件（持久化）
    print(f"\n正在保存到文件...")
    save_success = kg.save_to_file()
    if save_success:
        print(f"[OK] 知识图谱已成功保存到文件")
    else:
        print(f"[WARNING] 知识图谱保存到文件失败，但内存数据已更新")
