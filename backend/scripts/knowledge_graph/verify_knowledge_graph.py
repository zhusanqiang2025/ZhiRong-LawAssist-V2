...
def verify_knowledge_graph():
    ...
    # 按分类统计
    categories = {}
    for name, ct_def in kg._contract_types.items():
        # ContractTypeDefinition 是一个 dataclass
        category = getattr(ct_def, 'category', '未分类')
    ...
    for contract_name in test_contracts:
        if contract_name in kg._contract_types:
            ct_def = kg._contract_types[contract_name]
            lf = getattr(ct_def, 'legal_features', None)
            if lf:
                print(f"\n  {contract_name}:")
